...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val grouped = w.toLowerCase.groupBy((c: Char) => c)
    grouped.toList.sortBy(x => x._1).map(x => (x._1, x._2.length))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val xmap = x.toMap
    def subtractOccurrence(m: Map[Char, Int], o: (Char, Int)): Map[Char, Int] = {
      val (k, v) = o
      if ((m(k) - v) <= 0) m - k
      else m + ((k, m(k) - v))
    }
    y.foldLeft(xmap)(subtractOccurrence).toList.sortBy(x => x._1)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =  {
    def foo(oc: Occurrences): List[Sentence] = {
  ...
  def main(args: Array[String]) = {
    val test = List("Yes", "man")
    println(test)
    sentenceAnagrams(test).foreach(println)
  }
}
